...
class Masker():
    ...
    def __init__(self, width=5, mode='interpolate', infer_single_pass=False, include_mask_as_input=False):
...
def interpolate_mask(tensor, mask, mask_inv):
    ...
    for i in range(tensor.shape[1]):  # run c times or use group convolution
        filtered_tensor[:, i:i+1] = torch.nn.functional.conv2d(tensor[:, i:i+1], kernel, stride=1, padding=1)
